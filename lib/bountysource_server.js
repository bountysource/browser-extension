...
var BountysourceServer = {
  ...
  xhr: function(request, callback) {
    ...
    var path = BountysourceServer.api_url(request.path);
    if (request.params) {
      Object.keys(request.params).forEach(function(key, index) {
        path += (path.indexOf('?') === -1) ? '?' : '&';
        path += key + '=' + encodeURIComponent(request.params[key]);
      });
    }

    xhr.open(request.method || "GET", path, true);
