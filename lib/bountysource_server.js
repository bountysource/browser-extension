...
var BountysourceServer = {
  ...
  get_access_token: function(callback) {
    if (BountysourceServer.browser === 'chrome') {
      ...
      if (localStorage['safari_cookie']) {
        callback(BountysourceServer.get_access_token_from_cookie(localStorage['safari_cookie']));
      }
      callback(null);
    }
  },
  ...
  api: function(options) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4) {
        console.log("Call callback for path: ", options.request.path);
